...
The project covers the complete library workflow --- user authentication, book and catalogue management, borrowing and returns, reservations, reviews, and AI-powered book summaries.

The backend is organized into separate API routes, schemas, database models, and services to keep the code modular and maintainable.
...
-   Book CRUD operations with search, filtering and pagination
...
-   Database caching for generated book summaries
...
The project is divided into separate layers so that each part has a clear responsibility:

-   **`api/v1`** handles HTTP requests and API endpoints.
-   **`models`** contains the SQLAlchemy database models.
-   **`schemas`** handles request and response validation using
    Pydantic.
...
This separation keeps the code easier to understand, test and extend as new features are added.
...
These accounts are intended for local/assignment testing.
...
Supports book search, filtering and pagination.
...
Reservations include expiry handling.
...
If a summary already exists in the database, it is returned without making another AI API call. This helps reduce unnecessary API usage and response time.
...
Foreign keys and database constraints are used to maintain relationships between users, books, authors, categories and library transactions.
...
This project was built from a high-level library management requirement. The implementation focuses not only on CRUD operations but also on the complete workflows around borrowing, returns, reservations, reviews and AI summaries.

PostgreSQL is required for local development because the application uses PostgreSQL-specific features such as UUIDs and enum types.
